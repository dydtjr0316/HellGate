...
void CNetMgr::Do_Move(const int& user_id, const int& dir)
{
    ...
    cout << pClient->GetX() << endl;
    cout << pClient->GetY() << endl;
    cout << pClient->GetZ() << endl;
...
{
    cout << "Recv_Packet_Construct" << endl;
    ...
    while (rest_byte > 0) {
        ...
        if (packet_size <= rest_byte + pObj->Get_Prev_Size()) {
            cout << "패킷 사이즈" << packet_size << endl;
            cout << "남은 패킷 사이즈" << rest_byte << endl;
