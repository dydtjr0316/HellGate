	...
	, const char* _pMtrlFactorName)
{
	...
	Vector4 vRetVal = Vector4(vMtrl.mData[0] * dFactor, vMtrl.mData[1] * dFactor, vMtrl.mData[2] * dFactor, dFactor);
	return vRetVal;
}
...
wstring CFBXLoader::GetMtrlTextureName(FbxSurfaceMaterial* _pSurface, const char* _pMtrlProperty)
{
	...
	else if (m_fbxType == FBX_TYPE::ITEM) {
		wstrName += L"Texture\\Item\\";
	}
